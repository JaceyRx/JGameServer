            ...
            if (mapping != null) {
                ...
                for (int oneRpcNum : rpcNum) {
                    if (mappingMethodMap.containsKey(oneRpcNum)) {
                        log.error("【协议加载异常】 multiple actor method to handle same rpcNum = {}, method name = {} and {}",
                    ...
                    if (mappingActionClassMap != null && mappingActionClassMap.containsKey(oneRpcNum)) {
                        log.error(
                                "【协议加载异常】 multiple actor class and method to handle same rpcName {}, action class = {}, actor method = {}",
        ...
        if (o instanceof Terminated) {
            ...
            try {
                ...
                if (clazz != null) {
                    ...
                    IMessage response = (IMessage) methodAccess.invoke(this, mappingMethodMap.get(rpcNum), remoteMessage);
                    if (response != null) {
                        sender().tell(response, ActorRef.noSender());
                    }
                } else {
