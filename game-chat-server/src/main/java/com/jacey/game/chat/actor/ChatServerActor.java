...
import akka.actor.*;
import com.jacey.game.chat.manager.ConfigManager;
...
public class ChatServerActor extends UntypedAbstractActor {
    ...
    private ChatServerLoadBalanceService loadBalanceService = SpringManager.getInstance().getBean(ChatServerLoadBalanceServiceImpl.class);
    ...
    public void onReceive(Object o) throws Throwable {
        if (o instanceof Terminated) {
        ...
        } else if (o instanceof NetMessage) {		// 客户端请求
            ...
            switch (localMessage.getRpcNum()) {
                ...
                case LocalServer.LocalRpcNameEnum.LocalRpcRegistToGmServer_VALUE: {
                    ...
                    serverInfoBuidle.setServerType(CommonEnum.RemoteServerTypeEnum.ServerTypeChat);         // 服务器类型
                    serverInfoBuidle.setServerId(ConfigManager.CHAT_SERVER_ID);                             // 服务器id
                    serverInfoBuidle.setAkkaPath(ConfigManager.CHAT_SERVER_AKKA_PATH);                      // 服务器 akka path
            ...
            switch (remoteMessage.getRpcNum()) {
                case RemoteServer.RemoteRpcNameEnum.RemoteRpcRegistServer_VALUE: {
                    ...
                    if (errorCode == RemoteServer.RemoteRpcErrorCodeEnum.RemoteRpcOk_VALUE) {
                        ...
                        // 连接GM成功后，更新当前服务负载信息
                        loadBalanceService.setOneChatServerLoadBalance(ConfigManager.CHAT_SERVER_ID,
                                OnlineClientManager.getInstance().getBattleChatRoomCount());
                    ...
                    log.error("【RemoteMessage 解析错误】not support remoteMessage rpcNum = {}", remoteMessage.getRpcNum());
                    break;
                }
            }
        } else {
